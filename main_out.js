(function (wHandle, wjQuery) {
    ...
    function handleWheel(event) {
        zoom *= Math.pow(.9, event.wheelDelta / -120 || event.detail || 0);//修改視野縮放範圍
        0.5 > zoom && (zoom = 0.5);//設定最遠(越小越遠)
        zoom > 4 / viewZoom && (zoom = 4 / viewZoom) //設定最近(越大越近)
    }
...
for (var char, skin = ""; ;) {
    ...
    function drawGameScene() {
        ...
        if (0 != userScore) {
            ...
            scoreText.setValue('丸量: ' + ~~(userScore / 100));
            c = scoreText.render();
            a = c.width;
            ctx.globalAlpha = .2;
    ...
    function drawLeaderBoard() {
        ...
        if (null != teamScores || 0 != leaderBoard.length)
            if (null != teamScores || showName) {
                ...
                var scaleFactor = Math.min(0.22*canvasHeight, Math.min(200, .3 * canvasWidth)) / 170;
                ...
                var b;
                if (null == teamScores) {
                    for (ctx.font = "16px Ubuntu", b = 0; b < leaderBoard.length; ++b) {
                        ...
                        if (-1 != nodesOnScreen.indexOf(leaderBoard[b].id)) {
                            ...
                            ctx.fillStyle = "#5edff2";
                            if (!noRanking) {
                                c = b + 1 + ". " + c;
                          
                            }
                            ctx.fillText(c, 10, 70 + 26 * b); //靠左對齊//ctx.fillText(c, 100 - ctx.measureText(c).width / 2, 70 + 24 * b);//置中
                        } else {
                            ctx.fillStyle = "#FFFFFF";
                            if (!noRanking) {
                                c = b + 1 + ". " + c;
                            }
                            ctx.fillText(c, 10, 70 + 26 * b); //靠左對齊
                        }
                    }
                }
                else {
